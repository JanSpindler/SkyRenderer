...
float phase_m(float diff_cos, float g) {
	return (3*(1-g*g)*(1+diff_cos*diff_cos))/((4+2*g*g)*pow(1+g*g-2*g*diff_cos*diff_cos, 3.0f/2.0f));
}
...
vec3 transmittance(vec2 p1, vec2 p2, float r_scale_height, float m_scale_height, vec3 extcoeff_r, vec3 extcoeff_m, float rad_e) {
	...
	return exp(-(dens_m_sum*extcoeff_m + dens_r_sum*extcoeff_r));
}
...
float view_to_tex(float c_v, float h, float rad_e) {
	...
	if (c_v > c_h)
		return 0.5f*pow((c_v-c_h)/(1-c_h), 0.2f) + 0.5f;
	else
		return 0.5f*pow((c_h-c_v)/(1+c_h), 0.2f);
}
...
float tex_to_view(float u, float h, float rad_e) {
	...
	if (u > 0.5) {
		...
		return c_h - 2*u*  2*u*  2*u*  2*u*  2*u*(1 + c_h);
}
...
vec4 single_scattering(float h, vec2 view, vec2 light_in, float r_scale_height, float m_scale_height, vec3 scoeff_r, vec3 scoeff_m, vec3 extcoeff_r, vec3 extcoeff_m, float rad_e, float rad_a, float atmosphere_height, vec2 transmittance_resolution, sampler2D transmittance_texture) {
...
vec3 mie_from_rayleigh(vec4 r_rgb_m_r, vec3 scoeff_r, vec3 scoeff_m) {
	return r_rgb_m_r.rgb * r_rgb_m_r.a * scoeff_r.r * scoeff_m / (r_rgb_m_r.r * scoeff_m.r * scoeff_r);
}
